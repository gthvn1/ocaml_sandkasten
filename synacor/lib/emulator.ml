...
(** [decode vm] returns the four word (chunk) under IP that is the maximum size
    of an instruction. If only one word remains the last three can be None. If
    there is no word we are out or memory and an error is raised. *)
...
let decode (fetch_step : Insn.chunk * vm) : Insn.t * vm =
  ...
  | Some (insn, size) -> (insn, { vm with ip = vm.ip + size })

let execute (decode_step : Insn.t * vm) : vm =
  match decode_step with
  | Noop, vm -> vm
  | Halt, vm -> { vm with state = Halted }
  | Out c, vm ->
      Printf.printf "%c%!" c;
      vm
  | Unknown, vm ->
